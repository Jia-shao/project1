...
from PIL import Image
import io
def descri_img_by_qianwen(image_path, api_key,
                          model_name):
...
def single_reasoning_chain(image_path, text_prompt, instruction, api_key,
    ...
    while not answered and num <= 3:
        ...
        time_all += time1
        if answer_for_judge_dt or num == 3:
            ...
            if need_info == '1':
                ...
                time_all += time6
                result_code = run_code(position_code)
                edit_mask_path = result_code
                start_time = timer()
                edit_prompt = simplified_editing_prompt(edit_type=edit_type, instruction=instruction, api_key=api_key, model_name=model_name)
                end_time = timer()
...
def my_edit(image_path, save_path, instruction, middle_dir, device):
    chatmoss_api_key = ""
    device = device
    out_dir = middle_dir
